const fs = require('fs');
let code = fs.readFileSync('src/components/CreateTrackModal.tsx', 'utf8');
...
code = code.replace(
  "if (!topic.trim() || !commitmentTitle.trim() || !commitment.trim()) return;",
  "if (!isConfirmed) return;"
);

code = code.replace(
  "onSave(topic.trim(), commitmentTitle.trim(), commitment.trim());",
...
const formHtml = `      <div className="space-y-6">
        <div>
...
code = code.replace(/<div className="space-y-6">[\s\S]*?<\/div>\n    <\/Modal>/, formHtml + "\n    </Modal>");

fs.writeFileSync('src/components/CreateTrackModal.tsx', code);
